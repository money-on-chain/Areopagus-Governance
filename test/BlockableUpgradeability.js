...
const ProxyAdminChanger = artifacts.require('ProxyAdminChanger');
...
contract('BlockedUpgradeability', function([owner, anotherAddress]) {
  ...
  describe('GIVEN a ProxiableMockUpgradeable contract is up', function() {
    ...
    describe('AND the admin is set to an blocked BlockableUpgradeDelegator with a future unblock date', function() {
      ...
      describe('WHEN the owner tries to cjamge the contracts admin through governance', function() {
        let changeContract;

        beforeEach(async function() {
          const newAdmin = await ProxyAdmin.new();
          changeContract = await ProxyAdminChanger.new(
            proxyMockUpgradeable.address,
            delegator.address,
            newAdmin.address
          );
